...
8. Do not include any website name, logo, platform name, generated-by footer, or AI disclaimer inside the document.
...
function downloadPDF() {
  ...
  try {
    ...
    doc.text(DOC_TYPES[currentDocType].label.toUpperCase(), 105, 24, { align: 'center' });
    ...
    const cleanText = generatedDocText.replace(/\*\*/g, '').replace(/---+/g, '');
    const lines = doc.splitTextToSize(cleanText, maxWidth);
    let y = 38;
    const pageH = 275;
    lines.forEach(line => {
      if (y > pageH) { doc.addPage(); y = 24; }
    ...
    for (let i = 1; i <= pCount; i++) {
      ...
      doc.text(`Page ${i} of ${pCount}`, 105, 290, { align: 'center' });
    }

    const filename = `${DOC_TYPES[currentDocType].label.replace(/ /g,'_')}_${Date.now()}.pdf`;
    doc.save(filename);
    App.stats.saved++;
    App.saveStats();
